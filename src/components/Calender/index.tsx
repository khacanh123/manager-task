...
const App = () => {
    ...
    const [listTaskCalendar, setListTaskCanlendar] = useState<any>([])
  ...
  const [opened, setOpened] = useState(false);
    ...
    function getColorForLabel(text: string) {
    ...
    useEffect(() => {
        axios.post('/api/listwork').then((res) => {
            //    setListBoard(res.data.boards);
            setListBoard(res.data.data?.folders?.flatMap((item: any) => item.children).filter((item: any) => item.name.includes('Công Việc')));
    ...
    useEffect(() => {
        if (listboard.length > 0) {
            ...
            const query = `
        query {
  boards (ids: [${listboard.map((board: any) => board.id).join(" ")}]) {
    name
    state
    permissions
    items_page {
      items {
        ...
        `
            ...
            axios.post('/api/get-task', {
                ...
                const list = res.data.data?.boards.flatMap((i: any) => i.items_page.items);
                const arrTask: any[] = [];
                list.map((item: any) => {
                    const status = item?.column_values.filter((sub: any) => sub.id == 'task_status').length > 0 ? item.column_values.filter((sub: any) => sub.id == 'task_status')[0].text : ""
                    if (Array.isArray(item.subitems)) {
                        item.subitems.map((key: any) => {
                            arrTask.push(
                                {
                                    title: `[${key.name}] - ` + item?.name,
                                    color: getColorForLabel(status),
                                    id: item?.id,
                                    task_owner: item?.column_values.filter((sub: any) => sub.id == 'task_owner').length > 0 ? item.column_values.filter((sub: any) => sub.id == 'task_owner')[0] : null,
                                    task_status: item?.column_values.filter((sub: any) => sub.id == 'task_status').length > 0 ? item.column_values.filter((sub: any) => sub.id == 'task_status')[0] : null,
                                    task_priority: item?.column_values.filter((sub: any) => sub.id == "task_priority").length > 0 ? item.column_values.filter((sub: any) => sub.id == "task_priority")[0] : null,
                                    task_type: item?.column_values.filter((sub: any) => sub.id == "task_type").length > 0 ? item.column_values.filter((sub: any) => sub.id == "task_type")[0] : null,
                                    task_estimation: item?.column_values.filter((sub: any) => sub.id == "task_estimation").length > 0 ? item.column_values.filter((sub: any) => sub.id == "task_estimation")[0] : null,
                                    timelines: item?.column_values.filter((sub: any) => sub.id == "timeline5__1").length > 0 ? item.column_values.filter((sub: any) => sub.id == "timeline5__1")[0] : null,
                                    start: item?.column_values.filter((sub: any) => sub.id == "timeline5__1").length > 0 ? item.column_values.filter((sub: any) => sub.id == "timeline5__1")[0]?.text.split(" - ")[1] : null,
                                }
                            )
                        })
                    }else {
                        arrTask.push(
                            {
                                ...
                                color: getColorForLabel(status),
                                id: item?.id,
                                task_owner: item?.column_values.filter((sub: any) => sub.id == 'task_owner').length > 0 ? item.column_values.filter((sub: any) => sub.id == 'task_owner') : null,
                                task_status: item?.column_values.filter((sub: any) => sub.id == 'task_status').length > 0 ? item.column_values.filter((sub: any) => sub.id == 'task_status') : null,
                                task_priority: item?.column_values.filter((sub: any) => sub.id == "task_priority").length > 0 ? item.column_values.filter((sub: any) => sub.id == "task_priority") : null,
                                task_type: item?.column_values.filter((sub: any) => sub.id == "task_type").length > 0 ? item.column_values.filter((sub: any) => sub.id == "task_type") : null,
                                task_estimation: item?.column_values.filter((sub: any) => sub.id == "task_estimation").length > 0 ? item.column_values.filter((sub: any) => sub.id == "task_estimation") : null,
                                    start: item?.column_values.filter((sub: any) => sub.id == "timeline5__1").length > 0 ? item.column_values.filter((sub: any) => sub.id == "timeline5__1")[0]?.text.split(" - ")[1] : null,
                                    timelines: item?.column_values.filter((sub: any) => sub.id == "timeline5__1").length > 0 ? item.column_values.filter((sub: any) => sub.id == "timeline5__1") : null,
                            }
                        )
                    }
                })
